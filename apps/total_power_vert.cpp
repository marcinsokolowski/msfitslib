...
// CValueVector gIntTimeStamps;  
  ...
  if( fits_list.size() > 0 ){
     ...
     if( gEndChannel < 0 ){
        gEndChannel = first_fits.GetXSize()-1;
     ...
     for(int i=start_idx;i<fits_list.size();i++){
        ...
        if( fits.ReadFits( fits_list[i].c_str() ) ){
           ...
           if( gDebug > 0 ){
              printf("DEBUG : dumping total_power of lines in range %d - %d\n",gMinY,end_y);
           }
           
           for(int y=gMinY;y<end_y;y++){
              fits_processed++;
              int counter=0;
              long double total_sum=0.00,total_sum2=0.00,kurtosis=0.00;;
              long double total_sum_bedlam=0.00;
              double t_int = fits_time + fits.inttime*y;
              if( gIntTimeStamps.size() > 0 ){
                 t_int = gIntTimeStamps.get_uxtime_fast(y);
                 if( t_int <= 0 ){
              ...
              if( gExcludeNBrightestChannels>0 ){
                 ...
                 for(int x=gStartChannel;x<=end;x++){
                    double freq = fits.ch2freq(x);
                    if( gExcludedFreqRanges.size() > 0 ){
...
//                 printf("DEBUG1 : %.8f -> %.8f (units = %d)\n",double(power_in_units),double(total_sum),gOutUnit);
...
//              printf("DEBUG : %.8lf\n",total_sum_bedlam);
              ...
              if( fits.GetRangesCount() > 0 ){
                 int idx;
                 cIntRange* pRange = fits.GetRange(y,idx);
